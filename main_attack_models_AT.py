...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Evaluate robustness of AT models')
    
    # Add arguments
    parser.add_argument('--results_filename', '-f', type=str, default='results_attacks_AT.csv', help='Name of csv file to record results')
    ...
    parser.add_argument('--attack', '-a', type=str.lower, default='apgd_l2_dl', help='Attack to use', choices=attacks)
    parser.add_argument('--attack_train', '-at', type=str.lower, default='apgd_l2_dl', help='Attack used to train the models', choices=attacks)
    ...
    adv_list = [f'adveps_{i}' for i in args.adv_epsilon_list]

    # Write results to file
    perf_metrics = ['attack_success', 'original_acc', 'after_attack_acc', 'orig_f1_list', 'after_att_f1_list']
    column_names = ['split_num', 'db_name', 'attack', 'epsilon', 'norm_each_node', 'network_type', 'hidden_dim']
    for pm in perf_metrics:
        column_names += [str(pm) + '_' + str(i) for i in adv_list]
    ...
    for epsilon in args.epsilon_list:
        ...
        for split in range(args.num_splits):
            ...
            for it, eps_adv in enumerate(adv_list):
                # Load model
                mp = f"{model_path}_{eps_adv}_{args.attack_train}_no_constraint_{str(args.hidden_dim)}_split{str(split)}"
                ...
                print(f'{args.attack.upper()} {eps_adv} - Clean acc: {clean_acc:.2%} - Adv acc: {adv_acc:.2%}')
            ...
            # Prepare performance metrics for writing to CSV
            to_write_perf = list(att_success_list) + list(orig_acc_list) + list(after_att_acc_list) + list(orig_f1_list) + list(after_att_f1_list)
